...
@Transactional
    ...
    public ApiResponseGeneric registerProductionWaste(Long productId, double quantity, String reason){
        ...
        eventPublisher.publish(EventTopics.AUDIT, "INVENTORY", waste.getId().toString(), SecurityUtils.getUserId(), SecurityUtils.getUsername(), Map.of("action", "PRODUCTION_WASTE", "productId", productId, "quantity", quantity));
        return apiResponseGeneric("Production waste registered successfully");
    }

    private Inventory getOrCreateWaste() {
        return inventoryRepository.findByType(Type.WASTE)
                .orElseGet(() -> {
